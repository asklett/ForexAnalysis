...
- Include path name in __init__ arguments
...
class CurrencyPair():
    ...
    def __init__(self, details, period, interval, target_percent, signals):
        ...
        self.strategy_data = np.load('Data_30min/' + str(self.pair)+'.npy', allow_pickle=True)
    ...
    def correlation_analysis(self, cutoff_qty, cutoff_percentage):
        '''
        ...
        count = 0
        used_strategy_data = []
        for combination in [data[2] for data in self.strategy_data if
                            (data[1] >= cutoff_qty) and
                            (data[0]/data[1] >= cutoff_percentage/100)]:
            indices = combination
            combinedVarAND = True

            for j in indices:
                combinedVarAND &= signal_data.iloc[-1]['Signal_' + str(j)]

            if combinedVarAND > 0:
                count += 1
                used_strategy_data.append(combination)
            else:
                pass
            
        return count, used_strategy_data
...
while True:
    ...
    try:
        ...
        CONVERSION_TABLE_ERROR = 1

    # Loop through each currency pair and analyze
    if not CONVERSION_TABLE_ERROR:
        print('Starting next scan at ', str(datetime.now())[0:19])
        for index, row in ForexPairs.iterrows():
            try:
                # Check is strategy data is empty
                if len(pair_obj_list[index].strategy_data) == 0:
                    continue
                ...
                # Compare to strategy_data
                opportunity_count, strategy_data = pair_obj_list[index].correlation_analysis(CUTOFF_QTY, CUTOFF_PERCENT)
                ...
                if opportunity_count:
                    ...
                    # Log to file
                    logging.info(row['Pair'][0:6] + ', ' + str(units) + ' units, ' + str(pips) + ' pips. strategy_data: ' + str(strategy_data))
